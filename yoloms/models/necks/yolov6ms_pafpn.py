...
class YOLOv6MSPAFPN(YOLOv6RepPAFPN):
    ...
    Args:
        in_expand_ratio (float): Channel expand ratio for
            inputs of MS-Block. Defaults to 3.
        in_down_ratio (float): Channel down ratio for downsample conv layer
            in MS-Block. Defaults to 1.
        mid_expand_ratio (float): Channel expand ratio for each branch
            in MS-Block. Defaults to 2.
        layers_num (int): Number of layer in MS-Block. Defaults to 3.
        kernel_sizes (list(int, tuple[int])): Sequential or number of
            kernel sizes in MS-Block. Defaults to [1,3,3].
    """
    ...
    def __init__(self,
        ...
        self.layers_num = int(layers_num * kwargs['deepen_factor'])
    ...
    def build_top_down_layer(self, idx: int) -> nn.Module:
        ...
        if idx == 1:
            ...
            layer1 = ConvModule(in_channels=int(self.out_channels[idx - 1] *
                                                self.widen_factor),
    ...
    def build_bottom_up_layer(self, idx: int) -> nn.Module:
        ...
        """
        return MSBlock(int(self.out_channels[idx] * 2 * self.widen_factor),
                       int(self.out_channels[idx + 1] * self.widen_factor),
                       in_expand_ratio=self.in_expand_ratio,
                       in_down_ratio=self.in_down_ratio,
                       mid_expand_ratio=self.mid_expand_ratio,
