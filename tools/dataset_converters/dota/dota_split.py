...
def parse_args():
    ...
    parser.add_argument('split_config',
                        type=str,
                        help='The split config for image slicing.')
    parser.add_argument('data_root',
                        type=str,
                        help='Root dir of DOTA dataset.')
    parser.add_argument('out_dir',
                        type=str,
                        help='Output dir for split result.')
    parser.add_argument('--ann-subdir',
...
def crop_and_save_img(info, windows, window_anns, padding, padding_value,
    ...
    for window, ann in zip(windows, window_anns):
        ...
        cv2.imwrite(osp.join(save_dir, patch_info['id'] + '.' + img_ext),
                    patch)
...
def main():
    ...
    logger: MMLogger = MMLogger.get_instance('mmyolo',
                                             log_file=osp.join(
                                                 args.out_dir, log_file_name),
                                             log_level='INFO')
    ...
    for req_set in required_sets:
        ...
        infos = load_original_annotations(data_root=args.data_root,
